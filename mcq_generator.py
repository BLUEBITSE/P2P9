    ...
    def generate_mcqs_from_data(self, keyword_sentence_mapping, key_distractor_list):
        ...
        for keyword in key_distractor_list:
            ...
            if sentences:
                ...
                # Ensure there are exactly 4 options (3 distractors + 1 keyword)
                if len(distractors) < 4:
                    remaining_distractors = [d for d in key_distractor_list[keyword] if d != keyword]
                    if remaining_distractors:
                        random_distractor = random.choice(remaining_distractors)
                        distractors.append(random_distractor)

                # Shuffle the distractors to randomize their order
